...
- GitLab preserves the personal-project contract: public personal projects carrying
  the `profile-featured` topic take precedence; when none are tagged, it falls back
  deterministically to public personal non-fork projects.
- GitLab additionally accepts public group/subgroup projects returned by the
  user's contributed-projects feed only when they explicitly carry the
  `profile-featured` topic. Untagged non-personal contributions never enter
  `FEATURED PROJECTS`. GitLab's contributed-projects endpoint covers the past year,
  so this group-project discovery has the same rolling-window limitation.
- Personal and group candidates are deduplicated, then ordered by stars, recent
  activity, then name, with the shared six-card maximum.
...
- `FEATURED PROJECTS`: GitHub uses profile pinned repositories in their configured
  order. GitLab first applies the same explicit-topic/fallback selection to public
  personal projects, then adds only public group/subgroup projects from the
  past-year contributed-projects feed that explicitly carry `profile-featured`.
  Untagged external contributions are excluded. The combined set is deduplicated
  and ranked by stars, recent activity, then name. No second hand-maintained
  project list belongs in this file.
...
4. Register one project-local **Docker executor** runner named
   ...
   Working-branch pipelines run only checks that do not need protected publication
   secrets. Canonical publication, source-signature enforcement, source mirroring,
   and Profile Health remain on GitLab-hosted compute. This keeps the public project
   autonomous even when the local runner is offline.
...
this secret scan is the pre-integration CI check and runs on the project-local Docker
runner without protected publication credentials. The cryptographic source gate,
publication job, GitHub source mirror, and Profile Health reporter remain canonical
`dev`/release operations and therefore run on GitLab-hosted compute.
